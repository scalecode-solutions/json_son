  ...
  group('JsonSon Extensions', () {
    ...
    group('API Pattern Extensions', () {
      ...
      test('getPaginationInfo handles missing fields', () {
        final pagination = JsonSon({'page': 2, 'limit': 20});
      ...
      test('getApiError handles errors array', () {
        ...
        expect(error.userMessage,
            equals('Field name is required')); // Uses first error
      });
    ...
    group('Helper Classes', () {
      test('PaginationInfo calculates total pages', () {
        final info =
            PaginationInfo(total: 100, page: 2, limit: 25, hasMore: true);
      ...
      test('PaginationInfo handles edge cases', () {
        final info1 = PaginationInfo(page: 5, limit: 10);
        ...
        final info2 = PaginationInfo(total: 100, page: 10, limit: 10);
      ...
      test('ApiError provides user-friendly message', () {
        ...
        final error3 = ApiError(code: 'UNKNOWN');

        expect(
            error3.userMessage, equals('An unknown error occurred')); // Default
      });

      test('UserInfo provides full name', () {
        final user1 = UserInfo(firstName: 'John', lastName: 'Doe');

        expect(user1.fullName, equals('John Doe'));

        final user2 = UserInfo(name: 'Jane Smith');

        expect(user2.fullName, equals('Jane Smith'));

        final user3 = UserInfo(firstName: 'Alice');
