...
void main() {
  group('JsonSon Validator', () {
    test('validates required fields', () {
      final json = JsonSon({'id': 123, 'name': 'John', 'email': null});
    ...
    test('validates string fields', () {
      ...
      final validator = JsonSonValidator(json)
        ..custom('name', (value) => value is String,
            message: 'must be a string')
        ..custom('age', (value) => value is String, message: 'must be a string')
        ..custom('tags', (value) => value is String,
            message: 'must be a string');
    ...
    test('validates integer fields', () {
      final json = JsonSon({'id': 123, 'count': '456', 'name': 'John'});
    ...
    test('validates boolean fields', () {
      final json = JsonSon({'active': true, 'verified': 'yes', 'admin': 1});
    ...
    test('validates numeric fields', () {
      final json =
          JsonSon({'price': 29.99, 'quantity': '10', 'name': 'Product'});
    ...
    test('validates numeric ranges', () {
      final json = JsonSon({'age': 15, 'score': 110, 'quantity': 0});
    ...
    test('validates patterns', () {
      final json = JsonSon(
          {'zipcode': '12345', 'phone': '555-1234', 'code': 'ABC-123-XYZ'});
    ...
    test('validates allowed values', () {
      final json =
          JsonSon({'role': 'admin', 'status': 'pending', 'type': 'unknown'});
      ...
      final validator = JsonSonValidator(json)
        ..custom(
            ...
            (value) =>
                ...
                value.toString().contains(RegExp(r'\d')))
        ..custom(
            'confirmPassword', (value) => value == json.getString('password'))
        ..custom('evenNumber', (value) => value is int && value % 2 == 0,
    ...
    test('validates array items', () {
      ...
      expect(
          validator.errors['users[2].name'], contains('at least 1 characters'));
    });
    ...
    test('validates complex schema', () {
      final json = JsonSon({
        ...
        'inStock': true,
        'tags': ['new', 'featured'],
        'attributes': {'color': 'blue', 'weight': '2.5kg'},
        'ratings': [
          {'user': 'user1', 'score': 5},
      ...
      expect(validator.errors, hasLength(1));
      expect(
          validator.errors['ratings[1].score'], contains('must be an integer'));
    });
  });
}
