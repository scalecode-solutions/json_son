...
extension JsonSonApiExtension on JsonSon {
  ...
  PaginationInfo? getPaginationInfo({
    ...
    if (total != null || page != null || limit != null || hasMore != null || nextPage != null) {
  ...
  ApiError? getApiError({
    ...
    final details = getString(detailsKey) ?? 
                   getStringPath('error.details');
    ...
    if (errors is List) {
      errorsList = errors
          .map((e) => e is String ? e : (e is Map ? e['message']?.toString() : null))
          .where((e) => e != null)
          .cast<String>()
          .toList();
    }
  ...
  UserInfo? getUserInfo() {
    return UserInfo(
      ...
      name: getStringWithFallbacks(['name', 'username', 'displayName', 'display_name']),
      ...
      avatar: getStringWithFallbacks(['avatar', 'avatarUrl', 'avatar_url', 'profilePicture', 'profile_picture']),
    );
  }
  ...
  TimestampInfo getTimestampInfo({
    ...
    return TimestampInfo(
      createdAt: getDateTime(createdKey) ?? getDateTimePath('timestamps.created'),
      updatedAt: getDateTime(updatedKey) ?? getDateTimePath('timestamps.updated'),
      deletedAt: getDateTime(deletedKey) ?? getDateTimePath('timestamps.deleted'),
    );
  }
}
...
class PaginationInfo {
  ...
  @override
  String toString() => 'PaginationInfo(total: $total, page: $page, limit: $limit, hasMore: $hasMore, nextPage: $nextPage)';
}
...
class TimestampInfo {
  ...
  @override
  String toString() => 'TimestampInfo(createdAt: $createdAt, updatedAt: $updatedAt, deletedAt: $deletedAt)';
}
