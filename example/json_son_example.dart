...
void main() {
  ...
  List<int?>? parsedIntList = flexibleListFromJson<int>(listJsonValue, flexibleIntFromJson);
  ...
  print('Parsed List<int?> from single String "$listJsonValue": $parsedIntList');
  
  listJsonValue = 'invalid'; // Single invalid item
  parsedIntList = flexibleListFromJson<int>(listJsonValue, flexibleIntFromJson);
  print('Parsed List<int?> from single invalid String "$listJsonValue": $parsedIntList');
  ...
  List<String> parsedNonNullStringList = flexibleListNotNullFromJson<String>(nonNullListJsonValue, flexibleStringFromJson);
  print('Parsed List<String> from List $nonNullListJsonValue: $parsedNonNullStringList');
  
  nonNullListJsonValue = 'single_item';
  parsedNonNullStringList = flexibleListNotNullFromJson<String>(nonNullListJsonValue, flexibleStringFromJson);
  print('Parsed List<String> from single item "$nonNullListJsonValue": $parsedNonNullStringList');
  ...
  print('Parsed DateTime from ISO String "$dateTimeJsonValue": $parsedDateTime');

  dateTimeJsonValue = 1698316200000; // Milliseconds since epoch
  parsedDateTime = flexibleDateTimeFromJson(dateTimeJsonValue);
  print('Parsed DateTime from int timestamp $dateTimeJsonValue: $parsedDateTime');
  
  dateTimeJsonValue = '1698316200000'; // Milliseconds since epoch as string
  parsedDateTime = flexibleDateTimeFromJson(dateTimeJsonValue);
  print('Parsed DateTime from string timestamp "$dateTimeJsonValue": $parsedDateTime');
  ...
  List<String>? parsedCsvList = flexibleCommaSeparatedListFromJson(csvJsonValue);
  ...
  print('Raw ID: ${jsonData['id']} -> Parsed int: ${flexibleIntFromJson(jsonData['id'])}');
  print('Raw isActive: ${jsonData['isActive']} -> Parsed bool: ${flexibleBoolFromJson(jsonData['isActive'])}');
  print('Raw price: ${jsonData['price']} -> Parsed double: ${flexibleDoubleFromJson(jsonData['price'])}');
  print('Raw created_at: ${jsonData['created_at']} -> Parsed DateTime: ${flexibleDateTimeFromJson(jsonData['created_at'])}');
  print('Raw tags_string: ${jsonData['tags_string']} -> Parsed List<String>: ${flexibleCommaSeparatedListFromJson(jsonData['tags_string'])}');
  print('Raw values_list: ${jsonData['values_list']} -> Parsed List<num?>: ${flexibleListFromJson<num>(jsonData['values_list'], flexibleNumFromJson)}');
  print('Raw maybeANumber: ${jsonData['maybeANumber']} -> Parsed int: ${flexibleIntFromJson(jsonData['maybeANumber'])}');
  print('Raw singleValueAsList: ${jsonData['singleValueAsList']} -> Parsed List<String>: ${flexibleListNotNullFromJson<String>(jsonData['singleValueAsList'], flexibleStringFromJson)}');
